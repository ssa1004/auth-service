 ...
 */
class JwksAndOidcDiscoveryE2eTest : AbstractE2eTest() {

    private val http: HttpClient = HttpClient.newHttpClient()

    @Test
    fun `jwks endpoint 가 현재 키를 반환`() {
        val res = http.send(
    ...
    @Test
    fun `oidc discovery endpoint 가 표준 metadata 를 반환`() {
        val res = http.send(
            HttpRequest.newBuilder(URI.create("${baseUrl()}/.well-known/openid-configuration")).GET().build(),
            HttpResponse.BodyHandlers.ofString(),
        )
