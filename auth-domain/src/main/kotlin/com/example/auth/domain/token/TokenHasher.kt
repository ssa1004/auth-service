 ...
 *
 * BCrypt 가 아니라 평범한 hash 인 이유: refresh token 자체가 cryptographically random
 * (>= 256bit) 이라 brute-force 사전 공격이 불가능합니다. BCrypt 는 사람이 만든 약한 secret
 * 을 강화하기 위한 도구이고, 256bit random 값은 hash 한 번이면 충분합니다.
 *
 * 또한 refresh 검증은 매 API 요청마다 일어나는 hot path 라 BCrypt cost=12 (~250ms) 는
 * 부담입니다.
 */
object TokenHasher {
    ...
    fun sha256(token: String?): String {
        require(!token.isNullOrBlank()) { "token 은 비어있을 수 없습니다" }
        try {
            val md = MessageDigest.getInstance("SHA-256")
            val digest = md.digest(token.toByteArray(StandardCharsets.UTF_8))
            return HexFormat.of().formatHex(digest)
        } catch (e: NoSuchAlgorithmException) {
            throw IllegalStateException("JVM 에 SHA-256 가 없을 수 없습니다", e)
        }
    }
}
