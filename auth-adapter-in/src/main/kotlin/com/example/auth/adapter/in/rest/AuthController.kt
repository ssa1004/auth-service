 ...
 *
 * Spring Authorization Server 의 표준 OAuth2 endpoint (`/oauth2/token`,
...
class AuthController(
    ...
    fun register(
        ...
        val id = registerUserUseCase.register(
            RegisterUserUseCase.Command(
                req.tenantSlug, req.email, req.password, clientIpResolver.resolve(http),
            ),
        )
        return ResponseEntity.status(HttpStatus.CREATED).body(RegisterResponse(id.asString()))
    }
    ...
    fun login(
        ...
        return try {
            ...
            ResponseEntity.ok(TokenResponse.from(tokens))
        } catch (ex: MfaRequiredException) {
    ...
    fun verifyMfa(
        ...
        val tokens = verifyMfaUseCase.verify(
            VerifyMfaUseCase.Command(
                req.mfaToken,
                req.code,
                clientIpResolver.resolve(http),
                http.getHeader("User-Agent"),
    ...
    @ApiResponses(
        ...
        ApiResponse(
            responseCode = "401",
            description = "유효하지 않거나 이미 회전된 token (reuse 감지 시 사용자의 모든 세션 revoke)",
        ),
    ...
    fun refresh(
        ...
        val tokens = refreshTokenUseCase.refresh(
            RefreshTokenUseCase.Command(
