 ...
 *
 * <p>Spring Security 의 {@code oauth2Login()} 이 자동으로 다음 endpoint 를 노출:
 * <ul>
 *   <li>{@code /oauth2/authorization/{registrationId}} — IdP 로 redirect</li>
 *   <li>{@code /login/oauth2/code/{registrationId}} — IdP 의 callback</li>
 * </ul>
 *
 * <p>{@code spring.security.oauth2.client.registration.google.client-id} 가 없으면 본
 * config 가 비활성 → 기본 SecurityConfig 의 chain 만 작동 (테스트 환경 영향 없음).
 *
 * <p>order=0 으로 두어 기본 SecurityConfig 의 다른 chain 보다 먼저 매칭. matcher 가 oauth2
...
public class OidcLoginSecurityConfig {
    ...
    @Bean
    @Order(0)
    public SecurityFilterChain oidcLoginFilterChain(
            HttpSecurity http,
            OidcUserService customOidcUserService) throws Exception {
        http
