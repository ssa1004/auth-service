...
## 7. 보안 (auth-service 특화)

- **평문 비밀번호 / TOTP secret / refresh token 을 로그에 절대 출력 금지.** 디버깅도 금지.
  - 마스킹 기준: 비밀번호 / token 은 통째로 `***`. email 은 `u***@d***.com`.
- **JWK private key 는 application.yml 에 평문으로 두지 않는다.** 로컬 개발은 시드 키로
  한정, 운영은 Vault / KMS / k8s Secret 외부 주입.
- **DB 컬럼**: `users.password_hash` / `mfa_secrets.secret_cipher` 는 BCrypt / AES 후 저장.
  평문 컬럼 추가 금지.
- **테스트**: 실 사용자 데이터 / 운영 키 / 운영 JWK 를 fixture 에 넣지 말 것.
