...
## 배경
ADR-0008 에서 audit 는 *DB append-only* 로 박제. 운영 사고가 났을 때 보안팀은 *실시간*
분석을 위해 SIEM (Splunk / Elastic / Datadog) 도구를 쓴다. DB 만 적재하고 SIEM 으로
못 흘려보내면:

- 사고 인지가 늦음 (DB 스캔은 운영팀의 수동 쿼리).
- 룰 기반 알람 (예: "5분 안에 같은 IP 에서 100회 LOGIN_FAILED") 을 SIEM 이 못 봄.
- 컴플라이언스 (PCI-DSS / ISO 27001) 의 "tamper-resistant centralized logging" 요건 미흡.

직접 service 에서 Kafka 로 보내면 dual-write 문제:

- DB 트랜잭션 commit 직후 Kafka publish 가 실패하면 SIEM 에 안 흐름.
- Kafka publish 성공 후 트랜잭션 rollback 되면 SIEM 에는 있고 DB 에는 없음.

## 결정
**Outbox 패턴**.

1. `AuditLogRepositoryAdapter.append()` — *같은 트랜잭션* 에서 두 row INSERT:
...
2. `AuditOutboxPublisherWorker` — `@Scheduled(2s)` 폴링, 미발행 row 를 batch (50) 단위로
   `SiemEventPublisher.publish(topic, key, payloadJson)` 호출, 성공 시 `published_at` 박제.
3. 실패 시 `attempt_count` 증가 + `last_error` 기록. 5회 누적되면 폴링 대상에서 제외 →
   운영 알람 + 수동 dead-letter 처리 (후속).

at-least-once — publish 직후 published_at 박제 실패하면 다음 사이클에 같은 row 재발행 →
consumer 측에서 `eventId` 기반 dedup 필요.
...
- 운영 구현: Spring Kafka `KafkaTemplate` 기반 (별도 wiring — 본 ADR 시점에는 mock).
  Kafka 라이브러리를 본 모듈에 강제 의존시키지 않기 위해 인터페이스로 추상화.

## 결과
- audit 가 commit 되면 SIEM 도 *결국* 받는다 (at-least-once 보장).
- DB 와 Kafka 의 dual-write race 사고 차단.
