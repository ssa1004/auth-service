...
- **Token Introspection (RFC 7662)** — 다른 서비스가 발급 기관에 '이 토큰 지금도 살아있어?'라고 실시간으로 묻는 표준 방식. 토큰만 보고 혼자 판단하지 않고 원본에 직접 확인한다.
- **self-validate (자체 검증) JWT** — 검증자가 공개키만 있으면 발급 기관에 묻지 않고 토큰의 서명·만료를 혼자 확인하는 방식. 빠르지만 '발행 후 강제 종료'는 즉시 반영 못 하는 게 약점.
- **토큰 oracle (token oracle)** — introspect를 아무나 호출하게 열어두면, 공격자가 훔친 토큰이 유효한지 마음껏 물어볼 수 있는 '정답 확인기'가 돼버리는 위험. 그래서 인증된 client만 호출하게 막는다.
- **블록리스트 (blocklist, jti)** — 자체 검증되는 access 토큰은 따로 끌 방법이 없어, Redis에 '이 토큰 번호(jti)는 죽었음' 명단을 올려 검증 시 그걸 보고 차단하는 것. jti는 토큰마다 붙는 고유 일련번호.
- **fail-open vs fail-closed** — 보조 장치(Redis)가 고장 났을 때 fail-open은 '일단 통과'(가용성 우선), fail-closed는 '일단 막아'(안전 우선). 현재는 단순하게 fail-open이라 Redis가 죽으면 revoke 우회 여지가 있어 후속에서 보완 예정.
