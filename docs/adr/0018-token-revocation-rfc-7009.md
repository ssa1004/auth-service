...
- **Token Revocation (RFC 7009)** — 운영자가 '이 토큰 지금 즉시 끊어!'라고 강제로 정지시키는 표준 창구. 사용자가 직접 로그아웃하길 기다리지 않고 계정 정지·탈취 대응을 바로 한다.
- **정보 누설 차단 (200 OK 규칙)** — 가짜 토큰이든 진짜 토큰이든 다 똑같이 '200 OK'로만 답하는 규칙. "그게 유효한 토큰이었나, 가짜였나"가 응답으로 드러나면 공격자에게 힌트가 돼서 일부러 구별 안 해준다.
- **scope (token.revoke)** — client에게 부여하는 '권한 범위' 딱지. 이 딱지를 가진 admin client만 강제 revoke를 호출할 수 있어, 일반 서비스가 남의 토큰을 함부로 끄는 '서비스 거부 도구'가 되는 걸 막는다.
- **블록리스트 (blocklist, jti)** — 자체 검증되는 access 토큰은 따로 끌 방법이 없어, Redis에 '이 토큰 번호(jti)는 죽었음'을 토큰 남은 시간만큼 올려두고 검증 시 그걸 보고 차단하는 명단. jti는 토큰마다 붙는 고유 일련번호.
- **fail-closed (실패 시 거부)** — Redis 같은 차단 장치가 죽었을 때 '확실치 않으니 일단 막는다'로 가는 안전 정책. 현재는 단순 처리라 Redis가 죽으면 access 토큰 즉시 차단이 풀려, 후속에서 보완 예정.
- **mTLS (상호 인증)** — 비밀번호(client_secret) 대신 양쪽이 서로 인증서를 제시해 신원을 확인하는 더 강한 서버 간 인증. 평문 비밀번호 유출 위험이 없어 회사 내부망 표준으로 권장된다.
