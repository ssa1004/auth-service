...
- **부팅 느린 인스턴스가 죽음** — startup probe 없이 liveness probe 만 두면 Spring Boot
  + Flyway + JWK 초기 키 만들기까지 30초 걸리는 부팅을 *liveness 실패* 로 오인하고 K8s 가
  컨테이너를 영원히 재시작.
- **DB 일시 장애로 살아있는 pod 가 죽음** — liveness probe 가 *외부 의존* (DB / Redis) 까지
  포함하면, DB 가 잠깐 흔들렸을 때 살아있는 pod 들을 *동시에* 재시작 → cascade 사고 확대.
- **트래픽 받는데 의존이 down** — readiness probe 가 외부 의존을 *안* 보면, DB down 인
  pod 가 5xx 를 계속 뱉는 동안 K8s 는 트래픽을 계속 보냄.
...
- `@EventListener(ApplicationReadyEvent)` 에서 *JWK 초기 키 미생성* 시 `IllegalStateException`
  → 컨테이너 종료 코드 1 → K8s 재시작. (KMS 도입 후 KMS 응답 실패 케이스 대비)
...
- 연속 N회 (3) 임계값 — 외부 의존이 잠깐 흔들렸을 때마다 readiness 토글하면 노이즈가 큼.
...
## 결과
- 부팅 느려도 startup probe 가 잡고 있어 liveness 가 잘못 발동 안 함.
- DB 일시 장애로 살아있는 pod 가 cascade 재시작 사고 안 남.
- DB / Redis 가 꺼지면 5초 안에 readiness=DOWN → K8s 가 트래픽 차단 → 5xx 안 새 나감.
