...
| API4 | Unrestricted Resource Consumption | OK | `/login` `/register` `/refresh` `/verify-mfa` 모두 IP / username 별 token bucket |
...
- Refresh / login / register / verify-mfa 모두 IP 기반 rate limit (token bucket).
...
bucket4j-lettuce 기반 분산 token bucket (`RedisRateLimiterAdapter`) 으로 네 미인증 endpoint 모두
...
| `POST /api/v1/auth/verify-mfa` | `verify-mfa:<ip>` | 6자리 TOTP (10^6 공간) 추측 brute-force 차단 |
...
- `auth-application/.../VerifyMfaServiceTest.java#IP_있을_때_rate_limit_초과_시_RateLimited_예외_challenge_는_소비되지_않음`
...
- 2026-05-13 — 신규 작성. API4 / API8 의 새 가드 추가 (`register` / `refresh` rate limit, baseline
  보안 헤더) 와 함께.
- 2026-05-15 — API4 에 `verify-mfa` rate limit 추가. 마지막 남아 있던 미인증 endpoint 의 TOTP
  추측 brute-force 경로를 endpoint 자체 token bucket 으로 차단.
