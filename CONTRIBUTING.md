...
- Kotlin: IntelliJ Kotlin 공식 스타일 (`.editorconfig` 의 indent 4 칸 기준)
- 주석 / 문서는 자연스러운 한국어 (영어 직역체 지양)

## 보안 작업 시 추가 규칙

- 비밀번호 / TOTP secret / refresh token 을 평문으로 로그에 찍지 않습니다 (디버깅 포함).
  마스킹 기준은 비밀번호 / token 은 통째로 `***`, email 은 `u***@d***.com` 입니다.
- JWK / 비대칭 키 자료는 `.gitignore` 의 `*.pem`, `local-jwk.json` 등에 의해 항상 무시됩니다.
  운영에서는 application.yml 평문 보관 대신 KMS / Vault / k8s Secret 외부 주입을 사용합니다.
- DB 마이그레이션 (`V*__*.sql`) 으로 컬럼을 추가할 때 평문 비밀 컬럼은 받지 않습니다.
  `users.password_hash` 는 BCrypt, `mfa_secrets.secret_cipher` 는 AES-GCM 결과만 저장합니다.
- 테스트 fixture 에 실제 사용자 데이터 / 운영 키 / 운영 JWK 를 넣지 않습니다.
