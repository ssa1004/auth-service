 ...
 *
 * RBAC (역할 → 권한) 으로 표현하기 어려운 조건부 정책을 코드 밖 Rego 로 분리하여
 * 정책 변경이 코드 재배포로 이어지지 않도록 합니다 (ADR-0016).
 *
 * 호출 방식은 정책 별로 분리된 path — Rego 측 package 와 1:1 대응.
 * 예: `"auth/session/revoke"` → OPA path `data.auth.session.revoke.allow`.
 */
interface PolicyDecisionPort {
     ...
     * @param request    평가 입력.
     * @return 평가 결과. 구현체가 외부 호출에 실패해도 예외를 던지지 않고 `deny` 로
