 ...
 *
 * 구현체는 Kafka producer 또는 직접 SIEM HTTP API. 본 모듈은 *Kafka 매개* 구현
 * (auth.audit topic) 을 기본으로 두지만, 인터페이스만 보면 매개체에 비의존.
 *
 * at-least-once — publish 가 실패하면 호출자 (outbox worker) 가 재시도. 같은 이벤트가
 * 두 번 도달할 수 있어 consumer 측에서 eventId 기반 dedup 필요.
 */
interface SiemEventPublisher {
     ...
     */
    fun publish(topic: String, key: String, payloadJson: String)
}
